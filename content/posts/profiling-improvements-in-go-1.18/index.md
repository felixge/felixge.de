...
This change goes back to late 2019 (go1.13) when [Rhys Hiltner](https://github.com/rhysh) opened [GH 35057](https://github.com/golang/go/issues/35057) to report that he noticed a Go service that was using `20` CPU cores according to `top`, but only `2.4` cores according to Go's CPU profiler. Digging deeper, Rhys found that this problem was caused by dropped POSIX signals in the kernel.
